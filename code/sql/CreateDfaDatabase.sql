...
CREATE TRIGGER DFA_WORKFLOW_AFTER_INSERT AFTER INSERT ON DFA_WORKFLOW FOR EACH ROW BEGIN
	...
	select START_STATE_TYP, EVENT_TYP INTO STATE_TYP, EVENT_TYP FROM LKUP_WORKFLOW_TYP WHERE
...
CREATE TRIGGER DFA_WORKFLOW_STATE_BEFORE_INSERT BEFORE INSERT ON DFA_WORKFLOW_STATE FOR EACH ROW BEGIN
	...
	IF (NEW.IS_CURRENT = 1) THEN
		...
		UPDATE DFA_WORKFLOW_STATE SET IS_CURRENT = 0 WHERE DFA_WORKFLOW_STATE.IS_CURRENT = 1 
			AND DFA_WORKFLOW_STATE.DFA_WORKFLOW_ID = NEW.DFA_WORKFLOW_ID;
	ELSE
		SET NEW.IS_PASSIVE = 1;
		SELECT DFA_WORKFLOW_STATE.DFA_STATE_ID INTO CURRENT_STATE_ID FROM DFA_WORKFLOW_STATE 
			where DFA_WORKFLOW_STATE.DFA_WORKFLOW_ID = NEW.DFA_WORKFLOW_ID AND DFA_WORKFLOW_STATE.IS_CURRENT = 1;
...
CREATE TRIGGER DFA_WORKFLOW_STATE_AFTER_INSERT AFTER INSERT ON DFA_WORKFLOW_STATE FOR EACH ROW BEGIN
	...
	IF EXISTS (SELECT * FROM LKUP_WORKFLOW_STATE_TYP_CREATE lwstc WHERE lwtsc.STATE_TYP = NEW.STATE_TYP) THEN 
		...
		where lwtsc.STATE_TYP = NEW.STATE_TYP AND create_constraint.ALLOW_UPDATE=1 AND workflow_typ_constraint.ALLOW_UPDATE=1;	
